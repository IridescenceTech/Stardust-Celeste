...
#define NEEDBYTES(Num)                                                         \
  if (!CanReadBytes(Num))                                                      \
  return false // Check if at least Num bytes can be read from  the buffer,
               // return false if not
#define PUTBYTES(Num)                                                          \
  if (!CanWriteBytes(Num))                                                     \
  return false // Check if at least Num bytes can be written to the buffer,
               // return false if not

namespace Stardust_Celeste::Network {
ByteBuffer::ByteBuffer(size_t a_BufferSize)
    : m_Buffer(new char[a_BufferSize + 1]), m_BufferSize(a_BufferSize + 1),
      m_DataStart(0), m_WritePos(0), m_ReadPos(0) {}
...
inline uint64_t HostToNetwork8(const void *a_Value) {
  ...
  buf = (((static_cast<uint64_t>(htonl(static_cast<uint32_t>(buf)))) << 32) +
         htonl(buf >> 32));
  return buf;
}
...
bool ByteBuffer::ReadI8(s8 &a_Value) {
...
bool ByteBuffer::ReadU8(u8 &a_Value) {
...
bool ByteBuffer::ReadI16(s16 &a_Value) {
...
bool ByteBuffer::ReadU16(u16 &a_Value) {
...
bool ByteBuffer::ReadI32(int32_t &a_Value) {
...
bool ByteBuffer::ReadU32(uint32_t &a_Value) {
...
bool ByteBuffer::ReadI64(int64_t &a_Value) {
...
bool ByteBuffer::ReadU64(uint64_t &a_Value) {
...
bool ByteBuffer::ReadFloat(float &a_Value) {
...
bool ByteBuffer::ReadDouble(double &a_Value) {
...
bool ByteBuffer::ReadVarI32(uint32_t &a_Value) {
...
bool ByteBuffer::ReadVarI64(uint64_t &a_Value) {
...
bool ByteBuffer::WriteI8(int8_t a_Value) {
...
bool ByteBuffer::WriteU8(uint8_t a_Value) {
...
bool ByteBuffer::WriteI16(int16_t a_Value) {
...
bool ByteBuffer::WriteU16(uint16_t a_Value) {
...
bool ByteBuffer::WriteI32(int32_t a_Value) {
...
bool ByteBuffer::WriteU32(uint32_t a_Value) {
...
bool ByteBuffer::WriteI64(int64_t a_Value) {
...
bool ByteBuffer::WriteU64(uint64_t a_Value) {
...
bool ByteBuffer::WriteFloat(float a_Value) {
...
bool ByteBuffer::WriteDouble(double a_Value) {
...
bool ByteBuffer::WriteVarI32(uint32_t a_Value) {
...
bool ByteBuffer::WriteVarI64(uint64_t a_Value) {
...
bool ByteBuffer::ReadBuf(void *a_Buffer, size_t a_Count) {
  ...
  if (BytesToEndOfBuffer <= a_Count) {
    // Reading across the ringbuffer end, read the first part and adjust
    // parameters:
...
bool ByteBuffer::WriteBuf(const void *a_Buffer, size_t a_Count) {
  ...
  PUTBYTES(a_Count);
  const char *Src =
      static_cast<const char *>(a_Buffer); // So that we can do byte math
  ASSERT(m_BufferSize >= m_ReadPos);
...
bool ByteBuffer::ReadToByteBuffer(ByteBuffer &a_Dst, size_t a_NumBytes) {
  ...
  char buf[1024];
  // > 0 without generating warnings about unsigned comparisons where size_t is
  // unsigned
  while (a_NumBytes != 0) {
...
void ByteBuffer::ReadAgain(std::string &a_Out) {
  // Return the data between m_DataStart and m_ReadPos (the data that has been
  // read but not committed) Used by ProtoProxy to repeat communication twice,
  // once for parsing and the other time for the remote party
...
bool ByteBuffer::ReadVarUTF8String(std::string &a_Value) {
  ...
  if (Size > MAX_STRING_SIZE) {
    // LOGWARNING("%s: String too large: %u (%u KiB)", __FUNCTION__, Size, Size
    // / 1024);
  }
...
bool ByteBuffer::WriteVarUTF8String(std::string a_Value) {
  ...
  CheckValid();
  PUTBYTES(a_Value.size() + 1); // This is a lower-bound on the bytes that will
                                // be actually written. Fail early.
  bool res = WriteVarI32(static_cast<uint32_t>(a_Value.size()));
  if (!res) {
    return false;
  }
...
size_t ByteBuffer::GetVarIntSize(uint32_t a_Value) {
  ...
  do {
    // If the value cannot be expressed in 7 bits, it needs to take up another
    // byte
    Count++;
    a_Value >>= 7;
  } while (a_Value != 0);
