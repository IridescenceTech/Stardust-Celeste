...
bool ByteBuffer::ReadToByteBuffer(ByteBuffer &a_Dst, size_t a_NumBytes) {
    ...
    char buf[1024];
    // > 0 without generating warnings about unsigned comparisons where size_t
    // is unsigned
    while (a_NumBytes != 0) {
...
bool ByteBuffer::ReadVarUTF8String(std::string &a_Value) {
    ...
    if (Size > MAX_STRING_SIZE) {
        // LOGWARNING("%s: String too large: %u (%u KiB)", __FUNCTION__, Size,
        // Size / 1024);
    }
...
bool ByteBuffer::WriteVarUTF8String(std::string a_Value) {
    ...
    CheckValid();
    PUTBYTES(a_Value.size() + 1); // This is a lower-bound on the bytes that
                                  // will be actually written. Fail early.
...
size_t ByteBuffer::GetVarIntSize(uint32_t a_Value) {
    ...
    do {
        // If the value cannot be expressed in 7 bits, it needs to take up
        // another byte
        Count++;
        a_Value >>= 7;
    } while (a_Value != 0);
