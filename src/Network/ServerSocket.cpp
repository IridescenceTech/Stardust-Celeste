...
ServerSocket::ServerSocket(u16 port_number) {
    ...
    if (bind(my_socket, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) < 0) {
        throw std::runtime_error(
            ...
            std::to_string(port) + ". Errno: " + std::to_string(errno));
    }
...
auto ServerSocket::listen() const -> RefPtr<Connection> {
    ...
    auto conn =
        static_cast<int>(accept(my_socket, (struct sockaddr *)&socketAddress,
    ...
    return create_refptr<Connection>(conn);
}
} // namespace Stardust_Celeste::Network