...
MouseController::MouseController() { setup_scroll(); }
...
auto MouseController::update() -> void {
    ...
    for (const auto &[key, value] : command_map) {
        ...
        if (key.key >= 0 && key.key < 32) {
            ...
            mouseNow[key.key] = glfwGetMouseButton(GI::window, key.key);

            if ((key.flags & KeyFlag::Press && !mouseLast[key.key] &&
                 mouseNow[key.key]) ||
                (key.flags & KeyFlag::Held && mouseLast[key.key] &&
                 mouseNow[key.key]) ||
                (key.flags & KeyFlag::Release && mouseLast[key.key] &&
                 !mouseNow[key.key]) ||
                (key.flags & KeyFlag::Untouched && !mouseLast[key.key] &&
                 !mouseNow[key.key]))
...
auto MouseController::setup_scroll() -> void {
    ...
    glfwSetScrollCallback(
        GI::window, [](GLFWwindow *window, double x, double y) -> void {
            for (const auto &[key, value] : thizz->command_map) {
                ...
                } else if ((key.key == (int)MouseButtons::MWheelDown) &&
                           (y < 0)) {
                    // Value is negative, so adding it will properly contribute
                    // towards the negative treshold
                    current_scroll_value += y;
                    if (current_scroll_value < -scroll_wheel_threshold) {
                        current_scroll_value = 0;
                        value.func(value.data);
