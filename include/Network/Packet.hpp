...
class ByteBuffer : public NonCopy {
  public:
    ...
    /** Writes the bytes specified to the ringbuffer. Returns true if
...
#undef GetFreeSpace
#endif
    /** Returns the number of bytes that can be successfully written to the
     * ringbuffer */
    size_t GetFreeSpace(void) const;

    /** Returns the number of bytes that are currently in the ringbuffer. Note
     * GetReadableBytes() */
    size_t GetUsedSpace(void) const;

    /** Returns the number of bytes that are currently available for reading
     * (may be less than UsedSpace due to some data having been read already) */
    ...
    /** Skips reading by a_Count bytes; returns false if not enough bytes in the
     * ringbuffer */
    ...
    /** Reads the specified number of bytes and writes it into the destinatio
     * bytebuffer. Returns true on success. */
    ...
    /** Re-reads the data that has been read since the last commit to the
     * current readpos. Used by ProtoProxy to duplicate communication */
    void ReadAgain(std::string &a_Out);

    /** Checks if the internal state is valid (read and write positions in the
     * correct bounds) using ASSERTs */
...
struct PacketOut : public NonCopy {
    PacketOut(size_t size) : ID(0) { buffer = new ByteBuffer(size); }
    u32 ID;
    ByteBuffer *buffer;
};

struct PacketIn : public NonCopy {
    PacketIn(size_t size) : ID(0) { buffer = new ByteBuffer(size); }
    u32 ID;
    ByteBuffer *buffer;
};
} // namespace Stardust_Celeste::Network